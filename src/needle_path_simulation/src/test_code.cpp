...
int main(int argc, char **argv) {
    ros::init(argc, argv, "duration_example");
    ros::NodeHandle n;
    ros::Publisher pub = n.advertise<trajectory_msgs::JointTrajectoryPoint>("/test_topic", 1);

    trajectory_msgs::JointTrajectoryPoint t;

    ros::Duration duration1(5.25); // 5 seconds
    ros::Duration duration2 = ros::Duration(2.0); // 2 seconds

    t.time_from_start = duration1;
    
    ROS_INFO_STREAM("duration :" << duration1.toSec());
    ROS_INFO_STREAM("duration :" << duration2.toSec());
    ...
    // ROS_INFO_STREAM("Difference: " << diff.toSec());
    // ROS_INFO_STREAM("Multiplication: " << mult.toSec());
    // ROS_INFO_STREAM("Division: " << div.toSec());
    return 0;
}
...
//   test_pub.publish(my_plan.trajectory_);
...
//   // Test area
//   // ************************************
//   test_pub = node_handle.advertise<moveit_msgs::RobotTrajectory>("/test_topic", 5); 

//   // Set up to move to test point
//   waypoints.resize(0);
//   waypoints.push_back(move_group.getCurrentPose().pose);
//   geometry_msgs::Pose test_pose = move_group.getCurrentPose().pose;
//   test_pose.position.z -= 0.01;
//   waypoints.push_back(test_pose);
//   success = move_through_list_of_points(waypoints, move_group, joint_model_group, visual_tools, brain_model_pose, 1.0, 0.01, "Move to Test Pose", false);
//   if (!success) {
